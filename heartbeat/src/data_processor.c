...
int data_process(pthread_mutex_t *mutex_in_buffer, pthread_mutex_t *mutex_out_buffer, float *db_in,
                 uint8_t *db_out, uint8_t *data_ready, pthread_cond_t *input_cv)
{
    ...
    struct timespec ts;
    ts.tv_nsec = (1 / (float)(heartbeat_sampling_frequency * 2.2f)) * 1e6;
    ts.tv_sec = (1 / (float)(heartbeat_sampling_frequency * 2.2f)) / 1000;

    float ema_bpm_alpha = 1.0f - expf(-1.0f * heartbeat_ema_cutoff_sample_n);
    float ema_bpm_baseline = 240;

    float instantaneous_bpm = ema_bpm_baseline;
    ...
    while (keep_processor_running)
    {
        ...
        if (current_voltage > difference_thresh && !isUp)
        {
            ...
            instantaneous_bpm = 1.0f / (60 * (current_rise_ms - last_rise_ms) / 1000);
            ema_bpm = (1 - ema_bpm_alpha) * ema_bpm_baseline + ema_bpm_alpha * instantaneous_bpm;
        }
        else
        {
            isUp = 0;
        ...
        if (heartbeat_ready_counter == heartbeat_ema_cutoff_sample_n)
        {
